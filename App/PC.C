...
CPU_INT08U  OSTickDOSCtr;
             
static CPU_INT16U    PC_ElapsedOverhead;
static jmp_buf   PC_JumpBuf;
static CPU_BOOLEAN   PC_ExitFlag;
...
void PC_DispChar (CPU_INT08U x, CPU_INT08U y, CPU_INT08U c, CPU_INT08U color)
{
    ...
    offset  = (CPU_INT16U)y * DISP_MAX_X * 2 + (CPU_INT16U)x * 2;  /* Calculate position on the screen         */
    pscr    = (CPU_INT08U far *)MK_FP(DISP_BASE, offset);
...
*********************************************************************************************************
*/
void PC_DispClrCol (CPU_INT08U x, CPU_INT08U color)
{
    CPU_INT08U far *pscr;
    CPU_INT08U      i;


    pscr = (CPU_INT08U far *)MK_FP(DISP_BASE, (CPU_INT16U)x * 2);
...
*********************************************************************************************************
*/
void PC_DispClrRow (CPU_INT08U y, CPU_INT08U color)
{
    CPU_INT08U far *pscr;
    CPU_INT08U      i;


    pscr = (CPU_INT08U far *)MK_FP(DISP_BASE, (CPU_INT16U)y * DISP_MAX_X * 2);
...
*********************************************************************************************************
*/
void PC_DispClrScr (CPU_INT08U color)
{
    CPU_INT08U  far *pscr;
    CPU_INT16U      i;


    pscr = (CPU_INT08U far *)MK_FP(DISP_BASE, 0x0000);
...
void PC_DispStr (CPU_INT08U x, CPU_INT08U y, CPU_INT08U *s, CPU_INT08U color)
{
    ...
    offset  = (CPU_INT16U)y * DISP_MAX_X * 2 + (CPU_INT16U)x * 2;   /* Calculate position of 1st character     */
    pscr    = (CPU_INT08U far *)MK_FP(DISP_BASE, offset);
    while (*s) {
...
void PC_ElapsedStart(void)
{
    ...
    CPU_CRITICAL_ENTER();
    data  = (CPU_INT08U)inp(0x61);                              /* Disable timer #2                         */
    data &= 0xFE;
    outp(0x61, data);
...
CPU_INT16U PC_ElapsedStop(void)
{
    ...
    CPU_CRITICAL_ENTER();
    data  = (CPU_INT08U)inp(0x61);                                    /* Disable the timer                  */
    data &= 0xFE;
    outp(0x61, data);
    outp(TICK_T0_8254_CWR, TICK_T0_8254_CTR2_LATCH);             /* Latch the timer value              */
    low  = inp(TICK_T0_8254_CTR2);
    high = inp(TICK_T0_8254_CTR2);
    cnts = (CPU_INT16U)0xFFFF - (((CPU_INT16U)high << 8) + (CPU_INT16U)low); /* Compute time it took for operation */
    CPU_CRITICAL_EXIT();
    return ((CPU_INT16U)((CPU_INT32U)cnts * 54926L >> 16) - PC_ElapsedOverhead);
}
...
CPU_BOOLEAN PC_GetKey (CPU_INT16S *c)
{
    if (kbhit()) {                                         /* See if a key has been pressed            */
        *c = (CPU_INT16S)getch();                              /* Get key pressed                          */
        return (TRUE);
    } else {
...
void PC_SetTickRate (CPU_INT16U freq)
{
    ...
    if (freq == 18) {                            /* See if we need to restore the DOS frequency        */
        ...
        count = (CPU_INT16U)(((CPU_INT32U)2386360L / freq + 1) >> 1); 
    } else {
        count = 0;
    }
    CPU_CRITICAL_ENTER();
...
void *PC_VectGet (CPU_INT08U vect)
{
    ...
    pvect = (CPU_INT16U *)MK_FP(0x0000, vect * 4);        /* Point into IVT at desired vector location     */
    CPU_CRITICAL_ENTER();
...
void PC_VectSet (CPU_INT08U vect, void (*isr)(void))
{
    ...
    pvect    = (CPU_INT16U *)MK_FP(0x0000, vect * 4);     /* Point into IVT at desired vector location     */
    CPU_CRITICAL_ENTER();
    *pvect++ = (CPU_INT16U)FP_OFF(isr);                   /* Store ISR offset                              */
    *pvect   = (CPU_INT16U)FP_SEG(isr);                   /* Store ISR segment                             */
    CPU_CRITICAL_EXIT();
}
